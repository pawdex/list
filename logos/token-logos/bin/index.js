...
const { ChainId } = require("@pawdex/core-sdk");
  ...
  for (const chainId of Object.keys(ChainId)) {
    ...
    fs.readdir(path, (error, files) => {
      ...
      for (const token of files) {
        console.log(
          `Invalidating https://raw.githubusercontent.com/pawdex/list/master/logos/token-logos/network/${CHAIN_ID_TO_NAME[chainId]}/${token}`
        );
        exec(
          `cld uploader explicit "https://raw.githubusercontent.com/pawdex/list/master/logos/token-logos/network/${CHAIN_ID_TO_NAME[chainId]}/${token}" type="fetch" invalidate="true" eager='[{ "width": 24 }, { "width": 32 }, { "width": 48 }, { "width": 64 }, { "width": 96 }, { "width": 128 }]'`,
          () =>
            console.log(
              `Invalidated https://raw.githubusercontent.com/pawdex/list/master/logos/token-logos/network/${CHAIN_ID_TO_NAME[chainId]}/${token}`
            )
        );
      }
    });
  ...
  for (const chainId of Object.keys(ChainId)) {
    ...
    fs.readdir(path, (error, files) => {
      ...
      for (const token of files) {
        ...
        exec(
          `cld uploader explicit "https://raw.githubusercontent.com/pawdex/list/master/logos/token-logos/network/${CHAIN_ID_TO_NAME[chainId]}/${token}" type="fetch" invalidate="true"`,
          () =>
            console.log(
              `Invalidated https://raw.githubusercontent.com/pawdex/list/master/logos/token-logos/network/${CHAIN_ID_TO_NAME[chainId]}/${token}`
            )
...
program.command("invalidate:all-3").action(() => {
  ...
  fs.readdir(path, (error, files) => {
    ...
    for (const token of files) {
      console.log(
        `Invalidating https://raw.githubusercontent.com/pawdex/list/master/logos/token-logos/token/${token}`
      );
      exec(
        `cld uploader explicit "https://raw.githubusercontent.com/pawdex/list/master/logos/token-logos/token/${token}" type="fetch" invalidate="true"`,
        () =>
          console.log(
            `Invalidated https://raw.githubusercontent.com/pawdex/list/master/logos/token-logos/token/${token}`
          )
      );
    }
  });
...
program
  ...
  .action((network) => {
    ...
    fs.readdir(path, (error, files) => {
      ...
      for (const token of files) {
        console.log(
          `Invalidating https://raw.githubusercontent.com/pawdex/list/master/logos/token-logos/network/${NETWORK}/${token}`
        );
        exec(
          `cld uploader explicit "https://raw.githubusercontent.com/pawdex/list/master/logos/token-logos/network/${NETWORK}/${token}" type="fetch" invalidate="true" eager='[{ "width": 24 }, { "width": 32 }, { "width": 48 }, { "width": 54 }, { "width": 64 }, { "width": 96 }, { "width": 128 }]'`,
          () =>
            console.log(
              `Invalidated https://raw.githubusercontent.com/pawdex/list/master/logos/token-logos/network/${NETWORK}/${token}`
            )
        );
      }
    });
...
program
  ...
  .action((network, token) => {
    ...
    console.log(
      `Invalidating https://raw.githubusercontent.com/pawdex/list/master/logos/token-logos/network/${NETWORK}/${token}.jpg`
    );

    exec(
      `cld uploader explicit "https://raw.githubusercontent.com/pawdex/list/master/logos/token-logos/network/${NETWORK}/${token}.jpg" type="fetch" invalidate="true" eager='[{ "width": 24 }, { "width": 32 }, { "width": 48 }, { "width": 54 }, { "width": 64 }, { "width": 96 }, { "width": 128 }]'`,
      (error, stdout) => {
        if (error) {
          console.error(error);
        } else {
          console.log(stdout);
          console.log(
            `Invalidated https://raw.githubusercontent.com/pawdex/list/master/logos/token-logos/network/${NETWORK}/${token}.jpg`
          );
        }
      }
    );
...
program.command("explicit").action(() => {
  ...
  cloudinary.uploader.explicit(
    "https://raw.githubusercontent.com/pawdex/list/master/logos/token-logos/token/xsushi.jpg",
...
program.command("destroy").action(() => {
  console.log("destroy");
  cloudinary.uploader.explicit(
    "https://raw.githubusercontent.com/pawdex/list/master/logos/token-logos/network/ethereum/0x9cea2eD9e47059260C97d697f82b8A14EfA61EA5.jpg",
    { type: "fetch", invalidate: true },
    function (err, callResult) {
      console.log(err, callResult);
    }
